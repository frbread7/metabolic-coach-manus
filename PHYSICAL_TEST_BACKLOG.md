...
| PTB-V062-TR | `v0.6.2` / pending engineering commit | Phone | One naturally eligible prolonged-inactivity episode produces at most one WALK recommendation after post-meal and rapid-rise priority is applied; automated stairs never appear. | Matched pinned RC artifacts; walking reminders enabled; ordinary fresh/safe state | `NOT RUN` | `v0.7.0` RC |
| PTB-V062-AF | `v0.6.2` / pending engineering commit | Phone + Watch | Missing/blank-source, future, inconsistent, previous-day, cross-midnight, or stale activity context never leaves an actionable inactivity prompt. | Observe categories without recording health values or creating unsafe conditions | `NOT RUN` | `v0.7.0` RC; safety gate |
| PTB-V062-ID | `v0.6.2` / pending engineering commit | Phone + Watch + face | Routine refreshes retain one immutable episode/action, while new movement, activity-source change, settings mismatch, working-hours exit, expiry, source change, unsafe glucose, consumption, or an active session removes it without resurrection. | Naturally eligible prompt; pinned artifacts | `NOT RUN` | `v0.7.0` RC; identity/integrity gate |
| PTB-V062-AC | `v0.6.2` / pending engineering commit | Phone + Watch | A not-yet-applied delayed Start revalidates the latest activity context and fails closed after movement, expiry, or context change; an identical replay of an already applied Start remains idempotently applied and never creates a second WALK session. | Test phone and Watch Start separately against a naturally eligible prompt | `NOT RUN` | `v0.7.0` RC; command integrity |
| PTB-V062-SY | `v0.6.2` / pending engineering commit | Phone + Watch + face | The phone-authored inactivity recommendation, expiry, Start/Snooze result, reconnect, and completion converge through the existing Wear payload without duplicates or stale actions. | Pinned matched RC artifacts; paired devices | `NOT RUN` | `v0.7.0` RC |
| PTB-V062-BG | `v0.6.2` / pending engineering commit | Phone + Watch | Representative inactivity evaluation creates no extra polling cadence, background wake loop, notification storm, crash, ANR, excess heat, or obvious battery regression. | Normal daily configuration; compare categorically with prior accepted behavior | `NOT RUN` | `v0.7.0` RC; foundational if failed |
...
[docs/V0_6_2_INACTIVITY_WALK_COACHING.md](docs/V0_6_2_INACTIVITY_WALK_COACHING.md). The pending
commit placeholders above must be replaced with the final v0.6.2 feature commit when the
engineering gate is pinned. These rows will be consolidated into the promoted `v0.7.0` integrated
