...
6. the retained inactivity foundation;
7. notifications/quiet hours, snooze, cooldown, and the daily notification cap suppress the
   selected action when applicable.

Only post-meal and confirmed rapid-rise actions are enabled in `v0.6.1`; inactivity and stairs
remain future work. Post-meal wins if it overlaps a rapid rise. Rapid rise requires two consecutive
...
perform an extra provider request. Quiet hours suppress exercise actions but do not hide
...
longer satisfied, or a newer reading supersedes the rapid-rise pair. Start rechecks the same rapid
pair at tap time. A notification also times out at the action's validity boundary.
