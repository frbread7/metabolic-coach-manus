...
across daylight-saving changes. **Custom** accepts 14–90 completed local calendar days and must end
before today. Missing periods are shown as disconnected segments; coverage and largest-gap labels
explain how complete the period is.
...
Six-hour and twelve-hour selections intentionally show the existing insufficient-duration state,
not a numeric GMI or HbA1c-like estimate.
...
4. post-meal walk;
5. confirmed rapid-rise walk;
6. prolonged-inactivity WALK coaching;
...
If the required prior local reading is unavailable, no rapid prompt is created and the app does not
perform an extra provider request.

An inactivity prompt requires walking reminders, current same-day activity context, working hours,
and at least the configured inactivity duration. Missing, future, inconsistent, previous-day,
cross-midnight, or stale activity suppresses it. The existing glucose/activity freshness setting is
used conservatively for this gate. Automated inactivity coaching always suggests a walk;
enabling stair reminders does not turn it into an automated stair prompt. Manual stair quick
actions remain available and unchanged. Quiet hours suppress exercise actions but do not hide
higher-priority safety information generated by the phone.
...
quiet hours begin, a session is active, notifications are disabled, shared glucose safety is no
longer satisfied, or a newer reading supersedes the rapid-rise pair. Inactivity also disappears at
the activity-freshness boundary, outside working hours, or after new movement/activity-context
change. Start rechecks the rapid pair or latest inactivity episode before a session is created. A
notification or complication times out at the earliest of the action's validity boundary, the next
quiet-hours start, or—for inactivity—the working-hours end.
...
- rapid-rise threshold, exercise-pause fall rate, and shared glucose/activity freshness age;
